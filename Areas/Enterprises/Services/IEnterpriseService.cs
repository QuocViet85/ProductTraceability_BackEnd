using System.Security.Claims;
using App.Areas.Enterprises.DTO;
using App.Services;

namespace App.Areas.Enterprises.Services;

public interface IEnterpriseService : IBaseService<EnterpriseDTO>
{
    public Task<EnterpriseDTO> GetOneByTaxCode(string taxCode);
    public Task AddOwnerShipAsync(Guid id, Guid userId, ClaimsPrincipal userNowFromJwt);
    public Task GiveUpOwnershipAsync(Guid id, ClaimsPrincipal userNowFromJwt);
    public Task DeleteOwnershipAsync(Guid id, Guid userId);
}