...
public class FactoryRepository : IFactoryRepository
{
    ...
    public async Task<List<FactoryModel>> GetManyAsync(int pageNumber, int limit, string search)
    {
        ...
        List<FactoryModel> listFactories = await queryFactories.Include(f => f.User).Include(f => f.Enterprise).ToListAsync();
    ...
    {
        IQueryable<FactoryModel> queryFactories = _dbContext.Factories.Where(f => f.UserId == userId);
    ...
    {
        return await _dbContext.Factories.Where(f => f.UserId == userId).CountAsync();
    }
